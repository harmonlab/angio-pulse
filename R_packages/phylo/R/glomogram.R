...
replace.tip=function(phy, subtree, tip, split_age, tol=.Machine$double.eps^0.5){
...
# phy: the skeleton tree with tip labels corresponding to names in 'subtrees'
	...
	if(is.null(names(subtrees))) stop()
#	if(!all( names(subtrees)%in%phy$tip.label->check )){
#		stop(paste("The following subtrees were not matched by corresponding tips:\n", paste(paste("\t",names(subtrees)[!check],sep=""),collapse="\n")))
#	}
...
#				warning("ultrametricized 'phy'")
#			}
				phy=replace.tip(phy, curtree, cur$tip, cur$age, ...)		
			} else {
				warning("duplicate")
				next()
			}
